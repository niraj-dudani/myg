...
function init_solvers
	if ( { strcmp { METHOD } "hsolve" } == 0 )
		setup_hsolve { model_el } { CHANMODE }
	end
end
...
str global_file = { init_global_file "/tmp" }
str value_file
str spike_file
int line_num = 1
int iteration = -1
...
function iteration_begin
	...
	open_value_monitor { value_file } { IOCLOCK }
	
	spike_file = { spike_prefix } @ { iteration }
	open_spike_monitor { spike_file } { threshold } { refractory }
